...
async function experimentInit() {
  ...
  text_welcome = new visual.TextStim({
    win: psychoJS.window,
    name: 'text_welcome',
    text: 'Bienvenue dans la tâche de "n1-back" \n\nVous devez vérifier si la lettre présente correspond à celle présentée une lettre auparavant.\n \nexemple : S - S - B - M – M\n\nEn cas de correspondance, vous devez appuyer sur la gâchette droite. \n\n\nAppuyer sur la gâchette droite pour commencer.\n',
    font: 'Open Sans',
    units: undefined, 
  ...
  Fixation = new visual.TextStim({
    ...
    font: 'Open Sans',
    units: undefined, 
    pos: [0, 0], height: 10.10,  wrapWidth: undefined, ori: 0.0,
    languageStyle: 'LTR',
    color: new util.Color('white'),  opacity: undefined,
    depth: 0.0 
  });
  ...
  letter = new visual.TextStim({
    ...
    font: 'Open Sans',
    units: undefined, 
    pos: [0, 0], height: 10.15,  wrapWidth: undefined, ori: 0.0,
    languageStyle: 'LTR',
    color: new util.Color('white'),  opacity: undefined,
    depth: -1.0 
  });
...
function welcomeRoutineEachFrame() {
  return async function () {
    ...
    if (t >= 0.0 && mouse_resp_welcome.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
...
function endRoutineBegin(snapshot) {
  return async function () {
    ...
    for (let eachResp=0; eachResp<psychoJS.experiment._trialsData.length; eachResp++) {
        if (psychoJS.experiment._trialsData[eachResp]['corrAns']== '1') {
			nb_target += 1;
        ...
        if (psychoJS.experiment._trialsData[eachResp]['corrAns']!= 1) {
            nb_no_target += 1;
            if (psychoJS.experiment._trialsData[eachResp]['mouse_resp.leftButton']== '[1]'){
            nInCorr += 1;
            }
        }
    }
    ...
    - Nombre de réponses correctes : ${nCorr} sur ${nb_target}
    - Nombre d'erreurs:  ${nInCorr} sur ${nb_no_target}
    
    - Temps de réponse moyen (réponses correctes): ${meanRT} ms
	`
    ;	
